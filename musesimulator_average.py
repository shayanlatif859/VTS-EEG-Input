...
try:
    while True:
        ...
        # Collapse all EEG channels into a single averaged signal
        # (simple spatial averaging across electrodes)
        signal = np.mean(data[EEG_CHANNELS, :], axis=0).astype(np.float64)
        ...
        # Push raw signal to LSL for any other consumers
        raw_sample = [float(np.mean(data[ch, :])) for ch in EEG_CHANNELS]

        # Pad to four channels for LSL
        # This method was used. It used to look like lsl_sample = [float(np.mean(data[ch, :])) for ch in EEG_CHANNELS[:4]]
        # Also note that it was sliced to four channels. Now, EEG channels is set with the board.
